...
export const validateProduct = (values, err) => {
  let temp = {}
  temp.nama = /^[\w'\-,.][^0-9_!¡?÷?¿/\\+=@#$%ˆ&*(){}|~<>;:[\]]{2,}$/.test(values.nama) ? '' : 'Name can only be a letter of at least 3 characters'
...
export const validateProfile = (values, err) => {
  ...
  temp.nomor = /^[\+?\d*]{10,}$/.test(values.nomor) ? '' : 'Only filled with a minimum number of 10 characters'
  err({ ...temp })
}
